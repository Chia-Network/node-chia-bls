...
describe('HKDF', () => {
    ...
    for (const test of tests) {
        describe(test.ikm, () => {
            ...
            const ikm = Buffer.from(test.ikm, 'hex');
            const info = Buffer.from(test.info, 'hex');
            const prkExpected = Buffer.from(test.prkExpected, 'hex');
            const okmExpected = Buffer.from(test.okmExpected, 'hex');
    ...
    for (const test of tests) {
        describe(test.seed, () => {
            ...
            it('Has the correct master bytes', () =>
                assert.deepEqual(
                    master.toBytes(),
                    Buffer.from(test.masterSk, 'hex')
                ));
            it('Has the correct child bytes', () =>
                assert.deepEqual(
                    child.toBytes(),
                    Buffer.from(test.childSk, 'hex')
                ));
        });
    }
});
...
describe('XMD', () => {
    ...
    const dst = randomBytes(16);
    const ress: Map<Buffer, number> = new Map();
    it('Lengths are correct', () => {
        for (let length = 16; length < 8192; length++) {
            ...
            key = [...ress.keys()].find((buffer) => key.equals(buffer)) ?? key;
...
describe('SWU', () => {
    const dst_1 = Buffer.from(
        'QUUX-V01-CS02-with-BLS12381G2_XMD:SHA-256_SSWU_RO_',
        'utf-8'
    );
    const msg_1 = Buffer.from('abcdef0123456789', 'utf-8');
...
describe('Elements', () => {
    const i1 = bytesToBigInt(Buffer.from([1, 2]), 'big');
    const i2 = bytesToBigInt(Buffer.from([3, 1, 4, 1, 5, 9]), 'big');
    const b1 = i1;
    const b2 = i2;
...
describe('Chia vectors 1', () => {
    const seed1 = Buffer.from(Array(32).fill(0));
    const seed2 = Buffer.from(Array(32).fill(1));
    const msg1 = Buffer.from([7, 8, 9]);
    const msg2 = Buffer.from([10, 11, 12]);
    ...
    const msg3 = Buffer.from([1, 2, 3]);
    const msg4 = Buffer.from([1, 2, 3, 4]);
    const msg5 = Buffer.from([1, 2]);
...
describe('Chia vectors 3', () => {
    const seed1 = Buffer.from(Array(32).fill(4));
...
describe('Pyecc vectors', () => {
    const ref_sig1Basic = Buffer.from(
        [
    ...
    var secret1 = Buffer.from(Array(32).fill(1));
    var secret2 = Buffer.from(
        Array(32)
            .fill(0)
    ...
    var msg = Buffer.from([3, 1, 4, 1, 5, 9]);
...
describe('Readme', () => {
    ...
    const seed1 = Buffer.from([1, ...seed.slice(1)]);
    const sk1 = AugSchemeMPL.keyGen(seed1);
    const seed2 = Buffer.from([2, ...seed.slice(1)]);
    ...
    const seed3 = Buffer.from([3, ...seed.slice(1)]);
